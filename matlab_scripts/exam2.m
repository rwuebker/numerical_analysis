function[err,u] = exam2(N)
%
% Resolution de l'equation -u''(x)+x(u(x))^3=f(x)
...
while stop>1e-10
    ...
    b(1) = ???;
    for i=2:N-1
	    b(i) = coeff*(2*u(i)-u(i-1)-u(i+1))+???-f(i*h);
    end
    b(N) = ???;
...
% second membre de l equation -u''(x)+x(u(x))^3=f(x)
function f=f(x)
   %f=???;
   f=4*pi*pi*sin(2*pi*x)+exp(sin(2*pi*x));
end
% solution de l equation -u''(x)+x(u(x))^3=f(x)
function uex=uex(x)
   uex=sin(2*pi*x);
end
